...
$MY_URL = "?noviuscloud=apc";
$GLOBALS['MY_SELF']=$MY_SELF=
	"$MY_URL".
	...
	if (ADMIN_PASSWORD!='password' && ($MYREQUEST['LO'] == 1 || isset($_SERVER['PHP_AUTH_USER']))) {
		...
		if (!isset($_SERVER['PHP_AUTH_USER']) ||
			...
			echo <<<EOB
				...
				<big><a href='$MY_URL&OB={$MYREQUEST['OB']}'>Continue...</a></big>
				</body></html>
EOB;
			exit;
...
<div class="head">
	<h1 class="apc">
		<div class="logo"><span class="logo"><a href="<?= $MY_URL ?>">APC</a></span></div>
		<div class="nameinfo"><a href="http://pecl.php.net/package/APC">Opcode Cache</a></div>
	</h1>
	<div class="login">
	<?php put_login_link(); ?>
	</div>
...
EOB;
	...
	echo
		graphics_avail() ? 
			  '<tr>'.
			  "<td class=td-0><img alt=\"\" $size src=\"$MY_URL&IMG=1&$time\"></td>".
			  "<td class=td-1><img alt=\"\" $size src=\"$MY_URL&IMG=2&$time\"></td></tr>\n"
			: "",
		'<tr>',
...
EOB;
	...
	if (graphics_avail()) {
		...
		echo <<<EOB
			<img alt="" $size src="$MY_URL&IMG=3&$time">
EOB;
	}
	echo <<<EOB
